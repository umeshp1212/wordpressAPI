...
global $wpdb;
$myrows = $wpdb->get_results( "SELECT * FROM della_ws_synctime where id=1" );
$start_time=$myrows[0]->started_tstamp.'<br>';
$end_time=$myrows[0]->completed_tstamp;

$myrows = $wpdb->get_results( "UPDATE della_ws_synctime set started_tstamp='".$now_sql."' where id=1" );
...
$webservice = file_get_contents('http://erpwebservice.della.in:81/dellastore/onlinemglist.aspx?ProcType=1');
...
foreach($web_data as $web){
	...
	if($c_counter==$record_count){
		...
		$myrows = $wpdb->get_results( "UPDATE della_ws_synctime set completed_tstamp='".$end."' where id=1" );
	}
