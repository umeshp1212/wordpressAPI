 ...
 *
 * Template name: Sync delete Collection
 *
 * @package storefront
 */ 
...
global $wpdb;
$myrows = $wpdb->get_results( "SELECT * FROM della_ws_synctime where id=13" );
$start_time=$myrows[0]->started_tstamp.'<br>';
$end_time=$myrows[0]->completed_tstamp;

$myrows = $wpdb->get_results( "UPDATE della_ws_synctime set started_tstamp='".$now_sql."' where id=13" );
...
$webservice = file_get_contents('http://erpwebservice.della.in:81/dellastore/onlinemglist.aspx?ProcType=11');
...
foreach($web_data as $web){
	...
	if($start_time<$web_tstamp){
		...
		$CollectionCode=get_terms($CollectionCode_args);

		if(count($CollectionCode)>0){
			wp_delete_term($CollectionCode[0]->term_id,'product_cat');
			$query = "DELETE FROM della_ws_termmeta where term_id='".$CollectionCode[0]->term_id."' ";
