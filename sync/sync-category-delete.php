 ...
 *
 * Template name: Sync delete Category
 *
 * @package storefront
 */ 
...
global $wpdb;
$myrows = $wpdb->get_results( "SELECT * FROM della_ws_synctime where id=12" );
$start_time=$myrows[0]->started_tstamp.'<br>';
$end_time=$myrows[0]->completed_tstamp;

$myrows = $wpdb->get_results( "UPDATE della_ws_synctime set started_tstamp='".$now_sql."' where id=12" );
...
$webservice = file_get_contents('http://erpwebservice.della.in:81/dellastore/onlinemglist.aspx?ProcType=9');
...
foreach($web_data as $web){
	...
	if($start_time<$web_tstamp){
		...
		$CatCode=get_terms($CatCode_args);

		if(count($CatCode)>0){
			wp_delete_term($CatCode[0]->term_id,'product_cat');
			$query = "DELETE FROM della_ws_termmeta where term_id='".$CatCode[0]->term_id."' ";
